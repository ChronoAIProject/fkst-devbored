...
| Role | Default | Rule |
|---|---|---|
| Loop bot (`FKST_GITHUB_BOT_LOGIN`, trusted marker author) | `example-loop-bot` | writes markers/labels/PRs; must be repo collaborator |
| Human write actor (console's one write) | `example-human-actor` | must differ from bot after `[bot]` normalization (write-gate enforced); must satisfy `authorPolicy: collaborator` |
...
OPEN-DECISION: owner may substitute a dedicated bot account for `example-loop-bot`.
