...
const gitBlobHash = (bytes) => createHash('sha1')
  ...
  .update(bytes)
  .digest('hex');
const redactSecurityLiteral = (value, before, after) => {
  if (typeof value === 'string') return value.replaceAll(before, after);
  if (Array.isArray(value)) return value.map((item) => redactSecurityLiteral(item, before, after));
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value)
      .map(([key, item]) => [key, redactSecurityLiteral(item, before, after)]));
  }
  return value;
};
...
const securityRedaction = cutover.main_merge_security_redaction;
const registryText = readFileSync(resolve(root, map.requirement_registry), 'utf8');

const acceptsDocumentedSecurityRedaction = (path, expected, actual) => {
  const entry = securityRedaction?.files?.[path];
  if (!entry || entry.original_blob !== expected || entry.redacted_blob !== actual) return false;
  const original = execFileSync('git', ['cat-file', 'blob', expected], {
    cwd: root,
    maxBuffer: 10 * 1024 * 1024
  });
  const transformed = Buffer.from(original.toString('utf8')
    .replaceAll(securityRedaction.literal_before, securityRedaction.literal_after));
  if (gitBlobHash(transformed) !== actual) {
    fail(`security redaction for ${path} contains a change beyond the recorded literal replacement`);
  }
  return true;
};
...
for (const [path, expected] of Object.entries(cutover.historical_hash_guard)) {
  ...
  if (actual !== expected && !acceptsDocumentedSecurityRedaction(path, expected, actual)) {
    fail(`historical hash changed for ${path}: ${actual} != ${expected}`);
  }
}
...
let usedSquashedSnapshotFallback = false;
for (const [taskId, record] of Object.entries(openV1Records)) {
  let baselineBytes;
  let hasPreMutationBaseline = true;
  try {
    baselineBytes = execFileSync(
      'git',
      ['show', `${cutover.pre_mutation_head}:${record.path}`],
      { cwd: root, maxBuffer: 10 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] }
    );
  } catch {
    if (!securityRedaction?.squashed_public_snapshot_fallback) {
      fail(`${taskId} cannot read pre-mutation baseline ${cutover.pre_mutation_head}`);
    }
    baselineBytes = execFileSync('git', ['cat-file', 'blob', record.current_blob], {
      cwd: root,
      maxBuffer: 10 * 1024 * 1024
    });
    hasPreMutationBaseline = false;
    usedSquashedSnapshotFallback = true;
  }
  if (hasPreMutationBaseline && gitBlobHash(baselineBytes) !== record.cutover_blob) {
  ...
  const currentBlob = gitBlobHash(currentBytes);
  if (currentBlob !== record.current_blob
    && !acceptsDocumentedSecurityRedaction(record.path, record.current_blob, currentBlob)) {
    fail(`${taskId} current open-v1 blob lacks a documented re-baseline`);
  }
  const baseline = JSON.parse(baselineBytes.toString('utf8'));
  const current = JSON.parse(currentBytes.toString('utf8'));
  const comparableBaseline = redactSecurityLiteral(
    baseline,
    securityRedaction.literal_before,
    securityRedaction.literal_after
  );
  ...
  for (const key of new Set([...Object.keys(baseline), ...Object.keys(current)])) {
    if (!mutableOpenFields.has(key)
      && JSON.stringify(comparableBaseline[key]) !== JSON.stringify(current[key])) {
...
console.log(`open_v1_baseline=${usedSquashedSnapshotFallback ? 'pinned_public_snapshot_current_blobs' : 'pre_mutation_head'}`);
